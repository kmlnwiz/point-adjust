    ...
    function generateTable(data, order) {
        ...
        for (let i = 0; i < data['data'][order].length; i++) {
            html +=
                ...
                `<td class="text-center" scope="col"><input id="point-custom${order}-${i}" class="form-control form-control-sm text-center" type="number" placeholder="0" aria-label="" min="0" max="99999" value="${data['data'][order][i] === 0 ? '' : data['data'][order][i]}"></td>` +
                `</tr>`;
        };
